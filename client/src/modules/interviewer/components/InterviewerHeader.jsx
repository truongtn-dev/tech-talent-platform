...
const InterviewerHeader = ({ collapsed, setCollapsed, user }) => {
                ...
                <Space style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ textAlign: 'right', lineHeight: 1.3 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: '#111' }}>
                            {user?.profile?.fullName || user?.email?.split('@')[0] || 'Interviewer'}
                        </div>
                        <div style={{ fontSize: 12, color: '#6b7280' }}>Technical Expert</div>
                    </div>
                    <Dropdown
                        menu={{
                            items: [
